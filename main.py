...
class Game:
    ...
    def load_data(self):
        ...
        self.eletric_attack_img = pg.image.load(path.join(img_folder, ELETRIC_ATTACK_IMG)).convert_alpha()
        self.flying_attack_img = pg.image.load(path.join(img_folder,    FLYING_ATTACK_IMG)).convert_alpha()
        ...
        for hit in hits:
            ...
            if isinstance(hit, Pokemon):
                ...
                if isinstance(hits[hit][0], WaterAttack):
                    danodotiponovo = tipusManeiros.get('water', {}).get(hit.type, 1)

                    hit.health -= int(ATTACK_DAMAGE * danodotiponovo)
                if isinstance(hits[hit][0], FireAttack):
                    danodotiponovo = tipusManeiros.get('fire', {}).get(hit.type, 1)
                    hit.health -= int(ATTACK_DAMAGE * danodotiponovo)
                if isinstance(hits[hit][0], GrassAttack):
                    danodotiponovo = tipusManeiros.get('grass', {}).get(hit.type, 1)
                    hit.health -= int(ATTACK_DAMAGE * danodotiponovo)
                if isinstance(hits[hit][0], EletricAttack):
                    danodotiponovo = tipusManeiros.get('eletric', {}).get(hit.type, 1)
                    hit.health -= int(ATTACK_DAMAGE * danodotiponovo)
                if isinstance(hits[hit][0], FlyingAttack):
                    danodotiponovo = tipusManeiros.get('flying', {}).get(hit.type, 1)
                    hit.health -= int(ATTACK_DAMAGE * danodotiponovo)
                
                if isinstance(hits[hit][0], GroundAttack):
                    danodotiponovo = tipusManeiros.get('ground', {}).get(hit.type, 1)
                    hit.health -= int(ATTACK_DAMAGE * danodotiponovo)
