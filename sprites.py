        ...
        if self.is_controlled:
            ...
            if keys[pg.K_m] and self.in_battle:
                ...
                if now - self.last_attacked > TRAINED_ATTACK_DELAY:
                    ...
                    if self.type == 'eletric':
                        EletricAttack(self.game, self.pos, attack_vector.angle_to(X_AXIS), self.in_battle)
                    if self.type == 'flying':
                        FlyingAttack(self.game, self.pos, attack_vector.angle_to(X_AXIS), self.in_battle)
                    if self.type == 'ground':
                        GroundAttack(self.game, self.pos, attack_vector.angle_to(X_AXIS), self.in_battle)
                ...
                if self.game.battle_on:
                    if self.game.battle.pokemon_in:
                        ...
                        if now - self.last_attacked > WILD_ATTACK_DELAY:
                            ...
                            if self.type == 'eletric':
                                WildEletricrAttack(self.game, self.pos, attack_vector.angle_to(X_AXIS), self.in_battle)
                            
                            if self.type == 'flying':
                                WildFlyingAttack(self.game, self.pos, attack_vector.angle_to(X_AXIS), self.in_battle)
                            if self.type == 'ground':
                                WildGroundAttack(self.game, self.pos, attack_vector.angle_to(X_AXIS), self.in_battle)
...
class EletricAttack(Projectile):
    '''A child class of Projectile, with a new image and a type. '''
...
class WildEletricrAttack(EletricAttack):
    '''A child class of WaterAttack, with different groups for colliding with the player's pokemon instead of themselves. '''
...
class WildFlyingAttack(FlyingAttack):
    def __init__(self, game, pos, dir, in_battle):
         super().__init__(game, pos, dir, in_battle)
         self.game.battle.projectiles.remove(self)
         self.game.battle.wild_projectiles.add(self)

class GroundAttack(Projectile):
    def __init__(self, game, pos, dir, in_battle):
        super().__init__(game, pos, dir, in_battle)
        self.type = 'attack'
        self.image = self.game.grass_attack_img
class WildGroundAttack(GroundAttack):
    def __init__(self, game, pos, dir, in_battle):
         super().__init__(game, pos, dir, in_battle)
         self.game.battle.projectiles.remove(self)
         self.game.battle.wild_projectiles.add(self)
